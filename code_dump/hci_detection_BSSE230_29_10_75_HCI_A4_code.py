...
def run_eyes_detection(source, is_image=False):
    ...
    Args:
        ...
        is_image (bool): True if source is an image.
    """
    state_closure = {'blinks': 0, 'eye_closed': False, 'drowsy_frames': 0, 'awake_frames': 0}
        ...
        def process(frame, rgb_frame, paused, is_img):
            ...
            if not is_img and not paused and avg_ear > 0:
                if avg_ear < 0.25:
                    ...
                    state_closure['drowsy_frames'] += 1
                    state_closure['awake_frames']   = 0
                else:
                    if state_closure['eye_closed']:
                        state_closure['eye_closed'] = False
                        state_closure['blinks'] += 1
                    state_closure['awake_frames'] += 1
                    if state_closure['awake_frames'] >= 5:
                        state_closure['drowsy_frames'] = 0

            draw_ui_text(frame, f"Blinks: {state_closure['blinks']}", (10, 30))
            draw_ui_text(frame, f"L-EAR: {left_ear:.3f} | R-EAR: {right_ear:.3f}", (10, 70))

            if state_closure['drowsy_frames'] > 20:
                cv2.rectangle(frame, (0, 0), (w, 100), (0, 0, 255), -1)
                draw_ui_text(frame, "DROWSY! Wake Up!", (w // 2 - 150, 60), color=(255, 255, 255), font_scale=1.2, thickness=3)
        ...
        def process(frame, rgb_frame, paused, is_img):
            ...
            if res.multi_hand_landmarks:
                for hand_lm, handedness in zip(res.multi_hand_landmarks, res.multi_handedness):
                    ...
                    gesture = {0:"Fist", 1:"One", 2:"Peace", 3:"Three", 4:"Four", 5:"Open Hand"}.get(cnt, "Unknown")
                    if cnt == 1 and hand_lm.landmark[4].y < hand_lm.landmark[3].y and hand_lm.landmark[8].y > hand_lm.landmark[6].y:
                        gesture = "Thumbs Up"
                    if cnt == 5 and hand_lm.landmark[0].y * h < h / 2.0:
                        gesture = "High Five"
                        
                    current_gesture = gesture
                    gesture_display = f"{gesture} {GESTURE_EMOJI_MAP.get(gesture, '')}"
                    
                    draw_ui_text(frame, f"Fingers: {cnt} | {gesture_display}" if not is_img else gesture_display,
...
def main_menu():
    ...
    while True:
        ...
        elif choice == '2': run_eyes_detection(source, is_img)
        elif choice == '3': run_face_detection(source, is_img)
        elif choice == '4': run_hand_detection(source, is_img)
